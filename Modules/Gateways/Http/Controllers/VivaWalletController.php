...
class VivaWalletController extends Controller
{
    ...
    public function __construct(PaymentRequest $payment)
    {
        ...
        $this->source_code = 'Default';
        $this->config_mode = 'test';
        $this->token = '';

        if (!is_null($config) && $config->mode == 'live') {
            $this->config_values = json_decode($config->live_values) ?: (object) [];
            $this->config_mode = 'live';
        } elseif (!is_null($config) && $config->mode == 'test') {
            $this->config_values = json_decode($config->test_values) ?: (object) [];
            $this->config_mode = 'test';
        }

        if ($config && is_object($this->config_values)) {
            $this->client_id = $this->config_values->client_id ?? null;
            $this->client_secret = $this->config_values->client_secret ?? null;

            $sourceCodeKey = 'source_code';
            if (property_exists($this->config_values, $sourceCodeKey) && !empty($this->config_values->{$sourceCodeKey})) {
                $this->source_code = $this->config_values->{$sourceCodeKey};
            }
