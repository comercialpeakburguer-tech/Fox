...
Route::group(['prefix' => 'vendor', 'as' => 'restaurant.'], function () {
    ...
    // Fox GO - Stripe Connect onboarding embutido no cadastro do lojista
    Route::get('connect-onboarding', 'VendorController@foxgoConnectOnboarding')->name('connect_onboarding');
    Route::post('connect-account-session', 'VendorController@foxgoConnectAccountSession')->name('connect_account_session');
    Route::post('connect-status', 'VendorController@foxgoConnectStatus')->name('connect_status');
...
Route::get('/image-proxy', function () {
    ...
    if (
        $url === '' ||
        in_array(strtolower($url), ['null', 'undefined'], true) ||
...
// FOXGO_PAGARME_WEBHOOK_BEGIN
Route::any('/payment/pagarme/pay', [\Modules\Gateways\Http\Controllers\PagarmePaymentController::class, 'payment'])
    ->withoutMiddleware([\App\Http\Middleware\VerifyCsrfToken::class])
    ->name('pagarme.pay');

Route::get('/payment/pagarme/status', [\Modules\Gateways\Http\Controllers\PagarmePaymentController::class, 'status'])
    ->name('pagarme.status');


Route::any('/payment/pagarme/webhook', [\Modules\Gateways\Http\Controllers\PagarmePaymentController::class, 'webhook'])
    ->withoutMiddleware([\App\Http\Middleware\VerifyCsrfToken::class])
    ->name('pagarme.webhook');
// FOXGO_PAGARME_WEBHOOK_END

// Fox GO - Efí Pix Payment Gateway
Route::any('/payment/efi/pay', [\Modules\Gateways\Http\Controllers\EfiPaymentController::class, 'payment'])
    ->name('efi.pay');

Route::get('/payment/efi/status', [\Modules\Gateways\Http\Controllers\EfiPaymentController::class, 'status'])
    ->name('efi.status');

Route::any('/payment/efi/webhook', [\Modules\Gateways\Http\Controllers\EfiPaymentController::class, 'webhook'])
    ->name('efi.webhook');
// Fox GO - Efí Pix Payment Gateway
