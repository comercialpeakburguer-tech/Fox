...
if (! function_exists('order_place')) {
    function order_place($data) {
        ...
        // Fox GO - salva PaymentIntent Stripe no pedido para split Connect automático
        if (($data->payment_method ?? null) === 'stripe' && !empty($data->transaction_id) && str_starts_with((string) $data->transaction_id, 'pi_')) {
            if (DB::getSchemaBuilder()->hasColumn('orders', 'stripe_payment_intent_id')) {
                $order->stripe_payment_intent_id = $data->transaction_id;
            }
            if (DB::getSchemaBuilder()->hasColumn('orders', 'stripe_currency')) {
                $order->stripe_currency = strtolower($data->currency_code ?? 'brl');
            }
        }
