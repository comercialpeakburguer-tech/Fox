    ...
    public function store(Request $request)
    {
        ...
        if ($request->generic_name != null) {
            ...
            array_push($generic_ids, $generic_name->id);
                ...
                // Fox GO Logistics: Mercado exige perfil logístico no cadastro.
        $foxgoLogisticsErrors = \App\Services\FoxGo\ItemLogisticsProfileService::validationErrors($request, Helpers::get_store_data()->module->module_type);
        if (!empty($foxgoLogisticsErrors)) {
            return response()->json(['errors' => $foxgoLogisticsErrors]);
        }
...
$images = [];
        ...
        $this->foxGoV39SaveProductFields($request, $food);
        ...
        if (Helpers::get_mail_status('product_approval') && data_get($product_approval_datas, 'Add_new_product', null) == 1) {
            ...
            $food->is_approved = 0;
            $food->save();
        \App\Services\FoxGo\ItemLogisticsProfileService::saveFromRequest($request, $food);
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->generic_name != null) {
            ...
            array_push($generic_ids, $generic_name->id);
                ...
                // Fox GO Logistics: Mercado exige perfil logístico na edição.
        $foxgoLogisticsErrors = \App\Services\FoxGo\ItemLogisticsProfileService::validationErrors($request, Helpers::get_store_data()->module->module_type);
        if (!empty($foxgoLogisticsErrors)) {
            return response()->json(['errors' => $foxgoLogisticsErrors]);
        }
...
$p = Item::find($id);
        ...
        $this->foxGoV39SaveProductFields($request, $p);
        ...
        $p->save();
        \App\Services\FoxGo\ItemLogisticsProfileService::saveFromRequest($request, $p);
        $p->tags()->sync($tag_ids);
