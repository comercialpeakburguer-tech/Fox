...
class OrderController extends Controller
{
    ...
    {
        // Fox GO Logistics: aceitar pedido entra direto em preparo.
        // O botão antigo envia "confirmed"; para a Fox GO, aceite do lojista já significa "em preparo".
        $foxgoOriginalOrderStatus = $request->order_status ?? null;
        if ($foxgoOriginalOrderStatus === 'confirmed') {
        ...
        // Fox GO Logistics: fallback Food Bike para Moto sem subir para Carro.
        // Regra: delivery/food preso em Bike cai para Moto se não houver Bike online e houver Moto disponível.
        if (
            in_array(($request->order_status ?? null), ['processing', 'handover'], true)
            && $order
            && empty($order->delivery_man_id)
            && (int) ($order->dm_vehicle_id ?? 0) === 1
        ) {
            $foxgoModuleTypeForVehicleFallback = \Illuminate\Support\Facades\DB::table('modules')
                ->where('id', $order->module_id)
                ->value('module_type');

            if ($foxgoModuleTypeForVehicleFallback === 'food' && ($order->order_type ?? null) === 'delivery') {
                $foxgoMaxOrdersForFallback = (int) (config('dm_maximum_orders') ?: 2);

                $foxgoBikeOnline = \Illuminate\Support\Facades\DB::table('delivery_men')
                    ->where('active', 1)
                    ->where('status', 1)
                    ->where('vehicle_id', 1)
                    ->where('current_orders', '<', $foxgoMaxOrdersForFallback)
                    ->count();

                $foxgoMotoOnline = \Illuminate\Support\Facades\DB::table('delivery_men')
                    ->where('active', 1)
                    ->where('status', 1)
                    ->where('vehicle_id', 2)
                    ->where('current_orders', '<', $foxgoMaxOrdersForFallback)
                ...
                if ($foxgoBikeOnline <= 0 && $foxgoMotoOnline > 0) {
                    $order->dm_vehicle_id = 2;

                    $foxgoPayloadVehicleFallback = is_array($order->foxgo_logistics_payload ?? null)
                        ? $order->foxgo_logistics_payload
                        : [];

                    $foxgoPayloadVehicleFallback['vehicle_fallback'] = [
                        'from_vehicle_id' => 1,
                        'to_vehicle_id' => 2,
                        'reason' => 'food_bike_unavailable_moto_available',
                        'bike_online' => $foxgoBikeOnline,
                        'moto_online' => $foxgoMotoOnline,
                        'source' => 'vendor_order_status_processing_handover',
                        'applied_at' => now()->toDateTimeString(),
                    ];

                    $order->foxgo_logistics_payload = $foxgoPayloadVehicleFallback;
                }
            }
        }

// Fox GO Logistics: Mercado exige sacolas antes de pedido pronto.
        if (($request->order_status ?? null) === 'handover') {
            $foxgoOrderModuleType = \Illuminate\Support\Facades\DB::table('modules')
                ->where('id', $order->module_id)
                ->value('module_type');

            if ($foxgoOrderModuleType === 'grocery') {
                $foxgoBagCount = (int) $request->input('foxgo_bag_count', 0);
                $foxgoVolumeCount = (int) $request->input('foxgo_volume_count', 0);

                if ($foxgoBagCount <= 0 || $foxgoVolumeCount <= 0) {
                    \Brian2694\Toastr\Facades\Toastr::error('No Mercado, informe a quantidade de sacolas/volumes antes de marcar o pedido como pronto.');
                    return back();
                }

                $order->foxgo_bag_count = $foxgoBagCount;
                $order->foxgo_volume_count = $foxgoVolumeCount;

                $foxgoPayload = is_array($order->foxgo_logistics_payload ?? null)
                    ? $order->foxgo_logistics_payload
                    : [];

                $foxgoPayload['handover'] = [
                    'bag_count' => $foxgoBagCount,
                    'volume_count' => $foxgoVolumeCount,
                    'source' => 'vendor_handover',
                    'confirmed_at' => now()->toDateTimeString(),
                ];

                
                // Fox GO Logistics: gera OTP interno de retirada no handover - LEGADO_MERCADO.
                if (empty($order->foxgo_pickup_otp)) {
                    $order->foxgo_pickup_otp = str_pad((string) random_int(0, 9999), 4, '0', STR_PAD_LEFT);
                    $order->foxgo_pickup_otp_generated_at = now();
                }
...
// Fox GO Logistics: preserva confirmação quando o aceite virou preparo direto.
        if (($foxgoOriginalOrderStatus ?? null) === 'confirmed' && empty($order->confirmed)) {
            $order->confirmed = now();
        }

        // Fox GO Logistics: gera OTP interno de retirada para qualquer handover.
        if (($request->order_status ?? null) === 'handover' && empty($order->foxgo_pickup_otp)) {
            $order->foxgo_pickup_otp = str_pad((string) random_int(0, 9999), 4, '0', STR_PAD_LEFT);
            $order->foxgo_pickup_otp_generated_at = now();

            $foxgoAnyHandoverPayload = is_array($order->foxgo_logistics_payload ?? null)
                ? $order->foxgo_logistics_payload
                : [];

            $foxgoAnyHandoverPayload['pickup_otp'] = array_merge($foxgoAnyHandoverPayload['pickup_otp'] ?? [], [
                'generated' => true,
                'generated_at' => now()->toDateTimeString(),
                'source' => 'vendor_handover_any_module',
            ]);

            $order->foxgo_logistics_payload = $foxgoAnyHandoverPayload;
        }
