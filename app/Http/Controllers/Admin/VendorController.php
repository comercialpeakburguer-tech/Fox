...
class VendorController extends Controller
{
    ...
    private function getNewStores($request, $storeApproveStatus)
    {
        ...
        $stores = Store::with('vendor:id,f_name,l_name,email,phone,status,stripe_account_id,stripe_connect_status,stripe_charges_enabled,stripe_payouts_enabled,stripe_details_submitted,stripe_requirements_due', 'module:id,module_name', 'zone:id,name')->whereHas('vendor', function ($query) use ($storeApproveStatus) {
     ...
     private function updateVendorApplication($request)
    {
        ...
        // Fox GO - bloqueia aprovação se Stripe Connect estiver pendente/reprovada
        if ((int) $request->status === 1) {
            $foxgoVendor = $store->vendor;

            $foxgoStripePayload = json_decode($foxgoVendor?->stripe_requirements_due ?: '[]', true);
            $foxgoStripePayload = is_array($foxgoStripePayload) ? $foxgoStripePayload : [];

            $foxgoRequirements = $foxgoStripePayload['requirements'] ?? [];
            $foxgoCapabilities = $foxgoStripePayload['capabilities'] ?? [];

            $foxgoStripeActive =
                $foxgoVendor &&
                !empty($foxgoVendor->stripe_account_id) &&
                ($foxgoVendor->stripe_connect_status ?? null) === 'active' &&
                (int)($foxgoVendor->stripe_charges_enabled ?? 0) === 1 &&
                (int)($foxgoVendor->stripe_payouts_enabled ?? 0) === 1 &&
                (int)($foxgoVendor->stripe_details_submitted ?? 0) === 1 &&
                (($foxgoCapabilities['card_payments'] ?? null) === 'active') &&
                (($foxgoCapabilities['transfers'] ?? null) === 'active') &&
                empty($foxgoRequirements['disabled_reason'] ?? null) &&
                empty($foxgoRequirements['currently_due'] ?? []) &&
                empty($foxgoRequirements['past_due'] ?? []) &&
                empty($foxgoRequirements['pending_verification'] ?? []) &&
                empty($foxgoRequirements['errors'] ?? []);

            if (!$foxgoStripeActive) {
                $foxgoStripeMessage = 'Documento/identidade reprovado ou pendente pela Stripe. Oriente o lojista a corrigir nome, CPF/documento, data de nascimento, endereço, dados bancários e reenviar documento válido.';

                if (!empty($foxgoRequirements['errors'][0])) {
                    $foxgoStripeError = $foxgoRequirements['errors'][0];
                    $foxgoStripeCode = $foxgoStripeError['code'] ?? $foxgoStripeError['detailed_code'] ?? null;
                    $foxgoStripeRequirement = $foxgoStripeError['requirement'] ?? null;

                    if ($foxgoStripeCode === 'verification_failed_keyed_identity') {
                        $foxgoStripeMessage = 'Documento/identidade reprovado pela Stripe. Os dados digitados não bateram com a verificação. Oriente o lojista a corrigir nome, sobrenome, CPF/documento, data de nascimento e reenviar um documento válido que corresponda exatamente aos dados informados.';
                    } elseif ($foxgoStripeRequirement === 'individual.verification.document') {
                        $foxgoStripeMessage = 'Documento de identificação pendente ou recusado pela Stripe. Oriente o lojista a reenviar um documento válido e legível, com os mesmos dados informados no cadastro.';
                    } else {
                        $foxgoStripeMessage = 'Verificação Stripe pendente ou reprovada. Oriente o lojista a revisar os dados pessoais, documento e dados bancários.';
                    }
                } elseif (!empty($foxgoRequirements['currently_due'])) {
                    $foxgoStripeMessage = 'Pendência Stripe: ' . implode(', ', $foxgoRequirements['currently_due']);
                } elseif (!empty($foxgoRequirements['disabled_reason'])) {
                    $foxgoStripeMessage = 'Bloqueio Stripe: ' . $foxgoRequirements['disabled_reason'];
                }

                Toastr::error('Não é possível aprovar esta loja ainda. Stripe Connect não está liberada para repasse automático. ' . $foxgoStripeMessage);
                return back();
            }
        }
    ...
    // Fox GO - tela admin para corrigir verificação Stripe Connect
    public function foxgoStripeVerification($store)
    {
        $store = Store::withoutGlobalScopes()->with('vendor')->findOrFail($store);
        $vendor = $store->vendor;

        if (!$vendor || empty($vendor->stripe_account_id)) {
            Toastr::error('Esta loja ainda não possui conta Stripe Connect vinculada.');
            return back();
        }

        $stripePayload = json_decode($vendor->stripe_requirements_due ?: '[]', true);
        $stripePayload = is_array($stripePayload) ? $stripePayload : [];

        $receivingFields = [];
        $receivingMethod = \Illuminate\Support\Facades\DB::table('disbursement_withdrawal_methods')
            ->where('store_id', $store->id)
            ->where('is_default', 1)
            ->orderByDesc('id')
            ->first();

        if ($receivingMethod && !empty($receivingMethod->method_fields)) {
            $decoded = json_decode($receivingMethod->method_fields, true);
            $receivingFields = is_array($decoded) ? $decoded : [];
        }

        return view('admin-views.vendor.foxgo-stripe-verification', compact('store', 'vendor', 'stripePayload', 'receivingFields'));
    }

    // Fox GO - reenviar dados/documento para Stripe Connect
    public function foxgoStripeVerificationSubmit(Request $request, $store)
    {
        $store = Store::withoutGlobalScopes()->with('vendor')->findOrFail($store);
        $vendor = $store->vendor;
        ...
        $request->validate([
            ...
            'address_postal_code' => 'required|string|min:8|max:12',
            ...
            'document_front.required' => 'Envie a frente do documento.',
            'document_front.mimes' => 'A frente do documento deve ser JPG, PNG ou PDF.',
            'document_back.mimes' => 'O verso do documento deve ser JPG, PNG ou PDF.',
        ]);

        $secret = $this->foxgoFindStripeSecret();

        if (!$secret) {
            Toastr::error('Chave secreta da Stripe não encontrada nas configurações.');
        ...
        try {
            ...
            // Fox GO - Stripe exige telefone em formato internacional E.164.
            // Exemplo Brasil: 11916550478 -> +5511916550478
            ...
            $postalCode = preg_replace('/\D+/', '', (string) $request->address_postal_code);
            $state = strtoupper((string) $request->address_state);

            $frontFileId = $this->foxgoUploadStripeIdentityDocument($request->file('document_front'), $secret);
            $backFileId = $request->hasFile('document_back')
                ? $this->foxgoUploadStripeIdentityDocument($request->file('document_back'), $secret)
                : null;

            if (!$frontFileId) {
                Toastr::error('Não foi possível enviar o documento para a Stripe.');
                return back()->withInput();
            }

            $payload = [
                'individual[first_name]' => trim((string) $request->first_name),
                'individual[last_name]' => trim((string) $request->last_name),
                'individual[email]' => $vendor->email ?: $store->email,
                'individual[phone]' => $phone,
                'individual[id_number]' => $cpf,
                'individual[dob][day]' => $birthDate->day,
                'individual[dob][month]' => $birthDate->month,
                'individual[dob][year]' => $birthDate->year,
                'individual[address][line1]' => trim((string) $request->address_line1),
                'individual[address][city]' => trim((string) $request->address_city),
                'individual[address][state]' => $state,
                'individual[address][postal_code]' => $postalCode,
                'individual[address][country]' => 'BR',
            ...
            if (!$response->successful()) {
                info('FoxGoStripeVerification: erro update account vendor_id=' . $vendor->id . ' ' . $response->body());

                $message = data_get($response->json(), 'error.message') ?: 'A Stripe recusou a atualização dos dados.';
                Toastr::error('Erro Stripe: ' . $message);
                return back()->withInput();
            }

            $vendor->f_name = trim((string) $request->first_name);
            $vendor->l_name = trim((string) $request->last_name);
            $vendor->phone = $phone;
            $vendor->save();

            $this->foxgoSaveReceivingVerificationFields($store, [
                'foxgo_representative_cpf' => $cpf,
                'foxgo_representative_birth_date' => $birthDate->format('Y-m-d'),
                'foxgo_representative_address_line1' => trim((string) $request->address_line1),
                'foxgo_representative_address_city' => trim((string) $request->address_city),
                'foxgo_representative_address_state' => $state,
                'foxgo_representative_address_postal_code' => $postalCode,
                'foxgo_stripe_document_front_file_id' => $frontFileId,
                'foxgo_stripe_document_back_file_id' => $backFileId,
                'foxgo_stripe_document_resubmitted_at' => now()->toDateTimeString(),
            ]);

            \App\CentralLogics\FoxGoStripeConnectOnboardingLogic::refreshAccountStatus($store);

            Toastr::success('Dados e documento reenviados para a Stripe. Aguarde a nova análise antes de aprovar a loja.');
            return redirect()->route('admin.store.pending-requests');
        } catch (\Throwable $e) {
            info('FoxGoStripeVerification: exception store_id=' . $store->id . ' erro=' . $e->getMessage());

            Toastr::error('Erro ao reenviar verificação Stripe: ' . $e->getMessage());
            return back()->withInput();
        }
    }

    private function foxgoFindStripeSecret(): ?string
    {
        $rows = \Illuminate\Support\Facades\DB::table('addon_settings')
            ->where(function ($q) {
                $q->where('key_name', 'like', '%stripe%')
                    ->orWhere('key_name', 'like', '%payment%');
            })
            ->get();

        foreach ($rows as $row) {
            foreach (['live_values', 'test_values', 'additional_data'] as $field) {
                if (empty($row->$field)) {
                    continue;
                }
                ...
                while ($stack) {
                    ...
                    foreach ($item as $value) {
                        if (is_array($value)) {
                            $stack[] = $value;
                        } elseif (is_string($value) && str_starts_with($value, 'sk_')) {
                            return $value;
                        }
                    }
                }
    ...
    private function foxgoUploadStripeIdentityDocument($file, string $secret): ?string
    {
        ...
        if (!$response->successful()) {
            info('FoxGoStripeVerification: erro upload file ' . $response->body());
            return null;
        }
    ...
    private function foxgoSaveReceivingVerificationFields(Store $store, array $newFields): void
    {
        $method = \Illuminate\Support\Facades\DB::table('disbursement_withdrawal_methods')
            ->where('store_id', $store->id)
            ->where('is_default', 1)
            ->orderByDesc('id')
            ->first();
        ...
        \Illuminate\Support\Facades\DB::table('disbursement_withdrawal_methods')
            ->where('id', $method->id)
            ->update([
    ...
    // Fox GO - atualiza manualmente o status Stripe Connect da loja
    public function foxgoStripeVerificationRefresh($store)
    {
        $store = Store::withoutGlobalScopes()->with('vendor')->findOrFail($store);
        $vendor = $store->vendor;
        ...
        try {
            ...
            if (($vendor->stripe_connect_status ?? null) === 'active') {
                Toastr::success('Stripe Connect atualizado: conta liberada para cobrança e repasse.');
            } elseif (($vendor->stripe_connect_status ?? null) === 'pending_verification') {
                Toastr::info('Stripe Connect atualizado: documento recebido e ainda em análise pela Stripe.');
            } elseif (($vendor->stripe_connect_status ?? null) === 'verification_error') {
                Toastr::warning('Stripe Connect atualizado: ainda existe pendência ou reprovação na verificação.');
            } else {
                Toastr::info('Stripe Connect atualizado: verificação ainda pendente.');
            }

            return back();
        } catch (\Throwable $e) {
            info('FoxGoStripeVerificationRefresh: erro store_id=' . $store->id . ' erro=' . $e->getMessage());
            Toastr::error('Não foi possível atualizar o status Stripe agora: ' . $e->getMessage());
            return back();
        }
    }
