    ...
    public function business_index(Request $request, string $tab = 'business')
{
    ...
    switch ($tab) {
        ...
        case 'payment':
            $digital_payment_methods_count = Setting::whereIn('settings_type', ['payment_config'])
                ->whereIn('key_name', ['ssl_commerz', 'paypal', 'stripe', 'razor_pay', 'senang_pay', 'paytabs', 'paystack', 'paymob_accept', 'paytm', 'flutterwave', 'liqpay', 'bkash', 'mercadopago', 'efi'])
                ->where('is_active', 1)
                ->count();
    ...
    public function payment_config_update(Request $request)
    {
        ...
        $validation = [
            'gateway' => 'required|in:ssl_commerz,paypal,stripe,razor_pay,senang_pay,paytabs,paystack,paymob_accept,paytm,flutterwave,liqpay,bkash,mercadopago,efi',
        ...
        // Fox GO - efi gateway validation - INICIO
        if ($request['gateway'] == 'efi') {
            $additional_data = [
                'gateway_image' => 'nullable',
                'status' => 'required|in:1,0',
                'client_id' => 'required_if:status,1',
                'client_secret' => 'required_if:status,1',
                'certificate_path' => 'required_if:status,1',
                'pix_enabled' => 'nullable',
                'pix_cashin_enabled' => 'nullable',
                'pix_cashout_enabled' => 'nullable',
                'pix_open_finance_enabled' => 'nullable',
                'pix_key' => 'nullable',
            ...
            $validation_messages = [
                'client_id.required_if' => translate('Client ID is required when payment status is ON'),
                'client_secret.required_if' => translate('Client Secret is required when payment status is ON'),
                'certificate_path.required_if' => translate('Certificate path is required when payment status is ON'),
            ];
        }
        // Fox GO - efi gateway validation - FIM
        ...
        // Fox GO - sanitize efi gateway values - INICIO
        if ($request['gateway'] == 'efi') {
            unset($validated['gateway_image']);
        ...
        // Fox GO - sanitize efi gateway values - FIM

        $settings->live_values = $validated;
        $settings->test_values = $validated;
        ...
        if ($request->type == 'user_app') {
            ...
            if ($activationMode && $activationMode->value == 1 && $driveMondBaseUrl && $driveMondBaseUrl->value != null) {
                ...
                $response = Http::post($driveMondBaseUrl->value . '/api/store-configurations', [
                    'mart_business_name' => $name->value ?? 'Fox GO',
...
//        }
        elseif ($tab == 'business-section') {
            // FOXGO_FIX_BUSINESS_SECTION_SAVE
            $this->getAddLandingPageData($request, 'react_landing_page', 'business_title', true);
            $this->getAddLandingPageData($request, 'react_landing_page', 'business_sub_title', true);

            $business_image = \App\Models\DataSetting::where('type', 'react_landing_page')->where('key', 'business_image')->first();
            if ($business_image == null) {
                $business_image = new \App\Models\DataSetting;
                $business_image->key = 'business_image';
                $business_image->type = 'react_landing_page';
            }

            if ($request->hasFile('image')) {
                $business_image->value = \App\CentralLogics\Helpers::update(
                    'business_image/',
            ...
            \App\CentralLogics\Helpers::dataUpdateOrInsert(
                ['key' => 'download_business_app_links', 'type' => 'react_landing_page'],
                [
                    'value' => json_encode([
                        'seller_playstore_url_status' => $request->has('seller_playstore_url_status') ? 1 : 0,
                        'seller_playstore_url' => $request->seller_playstore_url,
                        'seller_appstore_url_status' => $request->has('seller_appstore_url_status') ? 1 : 0,
                        'seller_appstore_url' => $request->seller_appstore_url,
                        'dm_playstore_url_status' => $request->has('dm_playstore_url_status') ? 1 : 0,
                        'dm_playstore_url' => $request->dm_playstore_url,
                        'dm_appstore_url_status' => $request->has('dm_appstore_url_status') ? 1 : 0,
                        'dm_appstore_url' => $request->dm_appstore_url,
                    ]),
                ]
            );
