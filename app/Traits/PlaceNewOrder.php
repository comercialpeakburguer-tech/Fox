...
trait PlaceNewOrder
{
    ...
    public function new_place_order(Request $request, $is_prescription = false)
    {
        ...
        try {
            ...
            // FOXGO_FORCE_SERVER_DISTANCE_RECALC: recalcula a distancia no servidor para evitar frete antigo quando o cliente troca endereco no checkout.
            if ($request->order_type === 'delivery' && $store && $store->latitude && $store->longitude && $request->latitude && $request->longitude) {
                try {
                    $apiKey = BusinessSetting::where('key', 'map_api_key_server')->value('value');
                    if ($apiKey) {
                        $payload = [
                            'origins' => [[
                        ...
                        $meters = data_get($decoded, '0.distanceMeters');
                        if ($meters && is_numeric($meters) && $meters > 0) {
                            $request->merge(['distance' => round(((float) $meters) / 1000, 3)]);
                        }
                    }
                } catch (\Throwable $e) {
                    // Mantem a distancia enviada pelo app se a API externa falhar.
                }
            }
