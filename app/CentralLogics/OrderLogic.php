    ...
    public static function create_transaction($order, $received_by = false, $status = null)
    {
        ...
        try {
            ...
            try {
                ...
                // Fox GO - agenda split Stripe Connect após o ciclo da requisição.
                // Motivo: em alguns fluxos do 6amMart, create_transaction roda antes do order_status=delivered ser persistido.
                // No terminating hook, o FoxGoStripeConnectLogic recarrega o pedido do banco e só cria transfer se estiver stripe/paid/delivered e sem stripe_transfer_id.
                if (($order->payment_method ?? null) === 'stripe' && ($order->payment_status ?? null) === 'paid') {
                    $foxgoStripeOrderId = (int) $order->id;

                    app()->terminating(function () use ($foxgoStripeOrderId) {
                        try {
                            \App\CentralLogics\FoxGoStripeConnectLogic::executeForOrder($foxgoStripeOrderId);
                        } catch (\Throwable $e) {
                            info('FoxGoStripeConnect: erro terminating hook order_id=' . $foxgoStripeOrderId . ' erro=' . $e->getMessage());
                        }
                    });
                }
