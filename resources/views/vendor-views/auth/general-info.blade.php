    ...
    <section class="m-0 py-5">
        <div class="container">
            ...
            <div class="stepper">
                ...
                <div id="show-step-receiving" class="stepper-item">
                    <div class="step-name">Dados de recebimento</div>
                </div>
            ...
            <form id="form-id" enctype="multipart/form-data" novalidate>
                <div id="reg-form-div">
                                                ...
                                                <div class="card-body">
                                                    <div class="form-group mb-3">
                                                        ...
                                                        <input type="text" name="tin"
                                                               placeholder="{{ translate('Digite o CNPJ da loja') }}"
                                                               class=" form-control __form-control">
                                                    </div>
                                                    <div class="form-group mb-0">
                                                        <label class="input-label mb-2 d-block title-clr fw-normal"
                                                               for="exampleFormControlInput1">{{ translate('Data de abertura/emissão') }}
                                                        </label>
                ...
                @include('vendor-views.auth._foxgo_receiving_step')
...
$("#form-id").on('submit', function(e) {
    ...
    if (window.foxgoShouldBlockSubmitBeforeReceiving && window.foxgoShouldBlockSubmitBeforeReceiving()) {
        if (window.foxgoOpenReceivingStep) {
            window.foxgoOpenReceivingStep();
        }
        return false;
    }
...
function submitForm() {
    ...
    $.post({
        ...
        success: function (data) {
            ...
            // Fox GO - não aceitar HTML/string como sucesso de cadastro
            if (!data || typeof data !== 'object' || Array.isArray(data)) {
                $('.btn-disable').prop('disabled', false);
                console.error('Fox GO cadastro retornou resposta não JSON:', data);
                toastr.error('O cadastro não foi concluído. A resposta do servidor não veio em JSON. Veja o log/relatório antes de tentar novamente.');
                return;
            }
            if (data.errors) {
                $('.btn-disable').prop('disabled', false);
                ...
                setTimeout(function () {
                                        // Fox GO - redirect raiz seguro sem undefined
                    const foxgoRedirectUrl = data.redirect_url || data.redirectUrl || data.url || null;
                    const foxgoStoreId = data.store_id || data.storeId || null;
                    const foxgoBusinessPlan = data.business_plan || data.businessPlan || 'commission-base';

                    if (foxgoRedirectUrl && foxgoRedirectUrl !== 'undefined' && !String(foxgoRedirectUrl).includes('/undefined')) {
                        location.href = foxgoRedirectUrl;
                    } else if (foxgoStoreId) {
                        location.href = '/vendor/business-plan?store_id=' + encodeURIComponent(foxgoStoreId) + '&business_plan=' + encodeURIComponent(foxgoBusinessPlan);
                    } else {
                        $('#loading').hide();
                        console.error('Fox GO cadastro sem redirect_url/store_id:', data);
                        if (typeof toastr !== 'undefined') {
                            toastr.error('Cadastro recebido, mas não foi possível abrir a próxima etapa. Atualize a página e tente novamente.');
                        } else {
                            alert('Cadastro recebido, mas não foi possível abrir a próxima etapa.');
                        }
                    }
                }, 1000);
            }
        ...
        error: function (xhr, textStatus, errorThrown) {
            ...
            let msg = 'O cadastro não foi concluído. Corrija os dados e tente novamente.';
            let payload = null;
            ...
            if (payload && payload.errors) {
                if (Array.isArray(payload.errors)) {
                    payload.errors.forEach(function (err) {
                        toastr.error(err.message || err.value || 'Erro no cadastro.');
                    });
                    return;
                }
                ...
                Object.keys(payload.errors).forEach(function (key) {
                    ...
                    toastr.error(Array.isArray(value) ? value.join(' ') : String(value));
                });
                return;
            }

            if (xhr && xhr.responseText) {
                const clean = String(xhr.responseText).replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
                if (clean) {
                    msg = clean.substring(0, 220);
                }
            }

            console.error('Fox GO erro AJAX cadastro:', {
...
{{-- Fox GO - ajuste Brasil CNPJ CPF no lugar de NIF --}}
<script>
(function () {
    if (window.__foxgoNifToCnpjBrazil) return;
    window.__foxgoNifToCnpjBrazil = true;

    function replaceTextNode(node) {
        if (!node || node.nodeType !== Node.TEXT_NODE || !node.nodeValue) return;

        const map = {
            'NIF comercial': 'Documento fiscal da loja',
            'NIF Comercial': 'Documento fiscal da loja',
            'Número De Identificação Fiscal (NIF)': 'CNPJ da loja',
            'Número de Identificação Fiscal (NIF)': 'CNPJ da loja',
            'Certificado NIF': 'Comprovante do CNPJ / CCMEI',
            'certificado NIF': 'comprovante do CNPJ/MEI',
            'Data De Expiração': 'Data de abertura/emissão',
            'Data de Expiração': 'Data de abertura/emissão'
        };

        Object.keys(map).forEach(function (from) {
            if (node.nodeValue.indexOf(from) !== -1) {
                node.nodeValue = node.nodeValue.split(from).join(map[from]);
            }
        });
    }
    ...
    function fixPlaceholders() {
        document.querySelectorAll('input[name="tin"]').forEach(function (el) {
            el.placeholder = 'Digite o CNPJ da loja';
        });

        document.querySelectorAll('input[name="tin_expire_date"]').forEach(function (el) {
            const label = el.closest('.form-group')?.querySelector('label');
            if (label) label.textContent = 'Data de abertura/emissão';
        });

        document.querySelectorAll('input[name="tin_certificate_image"]').forEach(function (el) {
            const area = el.closest('.form-group, .col, .col-lg-4, .col-md-4') || document;
            walk(area);
        });
    }
...
{{-- Fox GO - correção Brasil loja usa CNPJ, CPF é do responsável --}}

{{-- Fox GO - UX público: remover abas de idioma e destravar endereço --}}
<style>
    .store-apply-navs,
    .store-apply-navs ~ .arrow-area,
    .tabs-slide-wrap .arrow-area {
        display: none !important;
    }
</style>
<script>
(function () {
    if (window.__foxgoVendorPublicUxFix) return;
    window.__foxgoVendorPublicUxFix = true;
    ...
    function fixLanguageTabs() {
        document.querySelectorAll('.store-apply-navs, .tabs-slide-wrap .arrow-area').forEach(function (el) {
            el.style.display = 'none';
        });

        document.querySelectorAll('.lang_form').forEach(function (el) {
            if (el.id === 'default-form') {
    ...
    function waitGoogle(callback, tries) {
        tries = tries || 0;
        ...
        if (tries < 40) {
            setTimeout(function () {
                waitGoogle(callback, tries + 1);
            }, 250);
        }
    }

    function applyMapLocation(result, typedAddress) {
        ...
        const pac = document.getElementById('pac-input');
        if (pac) {
            pac.value = result.formatted_address || typedAddress || '';
            trigger(pac);
        }

        // Não sobrescreve o campo principal de endereço.
        // O lojista pode apagar/corrigir normalmente.
        ...
        try {
            if (window.map && typeof window.map.setCenter === 'function') {
                ...
                if (typeof window.map.setZoom === 'function') window.map.setZoom(16);
            }

            if (window.marker && typeof window.marker.setPosition === 'function') {
                window.marker.setPosition(position);
            }

            if (window.restaurant_marker && typeof window.restaurant_marker.setPosition === 'function') {
                window.restaurant_marker.setPosition(position);
            }
        } catch (e) {}
    }
    ...
    function geocodeAddress(showToast) {
        ...
        waitGoogle(function () {
            ...
            geocoder.geocode({
                ...
                const currentTyped = (addressInput.value || '').trim();

                if (currentTyped !== typed) return;

                if (status === 'OK' && results && results.length) {
                    applyMapLocation(results[0], typed);
                    return;
                }
                ...
                if (showToast && window.toastr) {
                    toastr.error('Não foi possível localizar esse endereço. Corrija e tente novamente.');
                }
            });
        });
    }

    function fixAddressField() {
        const addressInput = document.getElementById('address');
        ...
        addressInput.placeholder = 'Digite o endereço completo da loja';

        if (pacInput) {
            pacInput.style.display = 'none';
            pacInput.setAttribute('tabindex', '-1');
            pacInput.setAttribute('aria-hidden', 'true');
        }
...
{{-- Fox GO - comprovante CNPJ CCMEI obrigatório visual --}}
<script>
(function () {
    if (window.__foxgoCcmeiRequiredVisual) return;
    window.__foxgoCcmeiRequiredVisual = true;
        ...
        document.body.querySelectorAll('*').forEach(function (el) {
            if (el.childNodes.length === 1 && el.childNodes[0].nodeType === Node.TEXT_NODE) {
                el.textContent = el.textContent
                    .replaceAll('Comprovante do CNPJ/MEI', 'Comprovante do CNPJ / CCMEI')
                    .replaceAll('Comprovante do CNPJ / MEI', 'Comprovante do CNPJ / CCMEI');
            }
        });
    }
