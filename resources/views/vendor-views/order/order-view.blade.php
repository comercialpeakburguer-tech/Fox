                                                        ...
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                ...
                                                                <button type="button" class="close"
                                                                        ...
                                                                        class="sr-only">Recusar pedido</span></button>
                                                            </div>
                        ...
                        <div class="row justify-content-md-end mb-3 mx-0 mt-4">
                            <div class="col-md-9 col-lg-8">
                                <dl class="row text-right">
                                    ...
                                    <dd class="col-6">
                                        {{ \App\CentralLogics\Helpers::format_currency($product_price + $del_c + $total_tax_amount + $total_addon_price + $additional_charge - $coupon_discount_amount - $store_discount_amount - $admin_flash_discount_amount  - $ref_bonus_amount + $extra_packaging_amount-$store_flash_discount_amount + $order->dm_tips - $order->extra_discount_amount) }}
                                    </dd>
                                    @if ($order?->payments)
                    ...
                    <div class="card mb-2">
                        ...
                        <div class="card-body pt-3">
                            ...
                            <div class="bg--secondary rounded p-10px">
                                <div class="mb-0">
                                    <div class="row g-1">
                                        <div class="foxgo-vendor-action-note">
                                            Fox GO: ao aceitar, o pedido entra direto em preparo. Quando finalizar, clique em <strong>Pedido pronto</strong> para chamar o motoboy para retirada.
                                        </div>
                                        <div class="{{ config('canceled_by_store') ? 'col-6' : 'col-12' }}">
                                            <a class="btn btn--primary w-100 fz--13 px-2 {{ $order['order_status'] == 'pending' ? '' : 'd-none' }} route-alert"
                                               ...
                                               data-message="{{ translate('messages.confirm_this_order_?') }}"
                                                href="javascript:">Aceitar pedido</a>
                                        </div>
                                        ...
                                        href="javascript:">{{ translate('messages.proceed_for_processing') }}</a>
                                        @endif
                                    <a class="btn btn--primary w-100 foxgo-handover-bags-alert {{ $order['order_status'] == 'processing' ? '' : 'd-none' }}"
                                       ...
                                       data-foxgo-module-type="{{ data_get($order, 'module.module_type') ?? \Illuminate\Support\Facades\DB::table('modules')->where('id', $order->module_id)->value('module_type') }}"
                                        {{-- Fox GO Logistics: fallback href real para Pedido pronto --}}
                                       href="{{ route('vendor.order.status', ['id' => $order['id'], 'order_status' => 'handover']) }}">Pedido pronto</a>

                                {{-- Fox GO Logistics: OTP retirada bloco seguro sem diretiva --}}
                                <div class="foxgo-pickup-otp-card mt-3 p-3 rounded"
                                     style="{{ (!empty($order->foxgo_pickup_otp) && in_array(($order['order_status'] ?? $order->order_status ?? ''), ['handover', 'picked_up'])) ? 'background:#fff7e6;border:1px solid #ffd591;' : 'display:none;' }}">
                                    <div style="font-weight:700;font-size:14px;margin-bottom:6px;">Código de retirada Fox GO</div>
                                    <div style="font-size:12px;color:#7a4b00;margin-bottom:10px;">
                                        Informe este código ao motoboy somente após conferir o pedido.
                                    </div>
                                    <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;">
                                        <span style="font-size:26px;font-weight:800;letter-spacing:6px;color:#111;background:#fff;padding:6px 12px;border-radius:8px;border:1px solid #ffd591;">
                                            {{ $order->foxgo_pickup_otp ?? '' }}
                                        </span>
                                        <span class="badge badge-soft-info"
                                              style="{{ empty($order->foxgo_pickup_otp_verified_at) ? '' : 'display:none;' }}">
                ...
                @if ($order['order_type'] != 'take_away' && $order->store->sub_self_delivery == 1)
                    ...
                    <div class="card mb-2">
                        ...
                        <!-- Body -->
                            ...
                            {{-- Fox GO Logistics: bloco OTP retirada entre configuracao e prova --}}
                            @if(!empty($order->foxgo_pickup_otp) && in_array(($order->order_status ?? ''), ['handover', 'picked_up']))
                                <div class="card mt-3">
                                    <div class="card-body">
                                        <h5 class="mb-2">Código de retirada Fox GO</h5>
                                        <p class="mb-2 text-muted" style="font-size: 12px;">
                                            Informe este código ao motoboy somente após conferir o pedido.
                                        </p>

                                        <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                                            <span class="badge badge-soft-warning px-3 py-2" style="font-size: 1.35rem; letter-spacing: 6px;">
                                                {{ $order->foxgo_pickup_otp }}
                                            </span>

                                            @if(!empty($order->foxgo_pickup_otp_verified_at))
                                                <span class="badge badge-soft-success">Código confirmado</span>
                                            @else
                                                <span class="badge badge-soft-info">Aguardando confirmação no app do entregador</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
...
<script>
    // Fox GO Logistics: prompt sacolas Mercado antes do handover.
    $(document).on('click', '.foxgo-handover-bags-alert', function (e) {
        e.preventDefault();

        const route = $(this).data('url');
        const message = $(this).data('message') || 'Marcar pedido como pronto';
        const moduleType = $(this).data('foxgo-module-type');
        ...
        Swal.fire({
            title: 'Pedido pronto - Mercado',
            html:
                '<div class="text-left">' +
                '<label class="form-label d-block">Quantidade de sacolas/volumes</label>' +
                '<input id="foxgo_bag_count" type="number" min="1" class="swal2-input" placeholder="Ex: 3">' +
                '<label class="form-label d-block mt-2">Volumes para retirada</label>' +
                '<input id="foxgo_volume_count" type="number" min="1" class="swal2-input" placeholder="Ex: 3">' +
                '<small>Esses dados alimentam a logística Fox GO e o app do motoboy.</small>' +
                '</div>',
            icon: 'info',
            showCancelButton: true,
            confirmButtonText: 'Confirmar pedido pronto',
            cancelButtonText: '{{ translate('messages.cancel') }}',
            preConfirm: () => {
                const bagCount = parseInt(document.getElementById('foxgo_bag_count').value || '0', 10);
                const volumeCount = parseInt(document.getElementById('foxgo_volume_count').value || '0', 10);

                if (bagCount <= 0 || volumeCount <= 0) {
                    Swal.showValidationMessage('Informe sacolas e volumes antes de marcar o pedido como pronto.');
                    return false;
                }

                return { bagCount, volumeCount };
            }
        }).then((result) => {
            if (result.isConfirmed || result.value) {
                const values = result.value;
                location.href = route + '&foxgo_bag_count=' + encodeURIComponent(values.bagCount) + '&foxgo_volume_count=' + encodeURIComponent(values.volumeCount);
            }
        });
    });
</script>
...
<style>
    /* Fox GO Logistics UI: botoes lojista simplificados */
    .foxgo-vendor-action-note {
        background: #fff7e6;
