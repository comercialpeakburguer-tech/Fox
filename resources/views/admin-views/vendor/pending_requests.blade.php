...
@section('content')
    <div class="content container-fluid">
        {{-- Fox GO - alerta simples Stripe Connect --}}
        @php
            $foxgoStripeAlertStores = collect(method_exists($stores, 'getCollection') ? $stores->getCollection() : $stores)
                ->filter(function ($foxgoStore) {
                    $foxgoVendor = $foxgoStore->vendor ?? null;

                    if (!$foxgoVendor || empty($foxgoVendor->stripe_account_id)) {
                        return false;
                    }

                    return ($foxgoVendor->stripe_connect_status ?? null) !== 'active'
                        || (int)($foxgoVendor->stripe_charges_enabled ?? 0) !== 1
                        || (int)($foxgoVendor->stripe_payouts_enabled ?? 0) !== 1;
                });
        @endphp

        @if($foxgoStripeAlertStores->count() > 0)
            <div class="alert alert-warning mb-3" role="alert">
                <strong>⚠️ Atenção Stripe Connect:</strong>
                uma ou mais lojas pendentes ainda não estão liberadas para receber repasse automático.
                Verifique documento, CPF, data de nascimento, endereço e dados bancários antes de aprovar/liberar split.
                <br>
                <small>
                    Lojas afetadas:
                    @foreach($foxgoStripeAlertStores as $foxgoStore)
                        #{{ $foxgoStore->id }} {{ $foxgoStore->name }}
                        @if($foxgoStore->vendor)
                            @php
                                $foxgoStripeStatusLabel = [
                                    'verification_error' => 'documento/identidade reprovado',
                                    'pending_verification' => 'em análise pela Stripe',
                                    'pending_verification' => 'em análise pela Stripe',
                                    'onboarding' => 'verificação pendente',
                                    'active' => 'ativo',
                                ][$foxgoStore->vendor->stripe_connect_status ?? ''] ?? 'pendente';
                            @endphp
                            — Stripe: {{ $foxgoStripeStatusLabel }} <a href="{{ route('admin.store.foxgo-stripe-verification', $foxgoStore->id) }}" class="btn btn-sm btn-outline-dark ml-2">Corrigir verificação Stripe</a>
                        @endif
                        @if(!$loop->last), @endif
                    @endforeach
                </small>
