    ...
    <div class="page-header pb-0">
        ...
        @php
            $foxgoVendor = $store->vendor ?? null;
            $foxgoHasStripeAccount = !empty(data_get($foxgoVendor, 'stripe_account_id'));
            $foxgoRepassesActive = $foxgoHasStripeAccount
                && data_get($foxgoVendor, 'stripe_connect_status') === 'active'
                && (int) data_get($foxgoVendor, 'stripe_charges_enabled', 0) === 1
                && (int) data_get($foxgoVendor, 'stripe_payouts_enabled', 0) === 1
                && (int) data_get($foxgoVendor, 'stripe_details_submitted', 0) === 1;

            $foxgoDisabledReason = data_get($foxgoVendor, 'stripe_requirements_disabled_reason');
            $foxgoRepassesTitle = 'Repasses ativos';
            $foxgoRepassesMessage = 'Esta loja está apta a receber repasses automáticos.';
            $foxgoRepassesAlertClass = 'alert-success';

            if (!$foxgoRepassesActive) {
                $foxgoRepassesAlertClass = 'alert-warning';
                $foxgoRepassesTitle = 'Atenção nos repasses';

                if (!$foxgoHasStripeAccount) {
                    $foxgoRepassesMessage = 'Esta loja ainda não possui conta de repasse vinculada.';
                } elseif (!empty($foxgoDisabledReason)) {
                    $foxgoRepassesAlertClass = 'alert-danger';
                    $foxgoRepassesTitle = 'Repasses bloqueados';
                    $foxgoRepassesMessage = 'A conta de repasse desta loja está bloqueada ou possui exigências pendentes.';
                } else {
                    $foxgoRepassesMessage = 'Esta loja ainda possui pendências de verificação para receber repasses automáticos.';
                }
            }
        @endphp

        <div class="alert {{ $foxgoRepassesAlertClass }} d-flex align-items-start gap-2 mb-3" role="alert">
            <i class="tio-wallet-outlined mt-1"></i>
            <div>
                <strong>Status de Repasses: {{ $foxgoRepassesTitle }}</strong>
                <div>{{ $foxgoRepassesMessage }}</div>
                @if(!$foxgoRepassesActive)
                    <small class="d-block mt-1">
                        Verifique cadastro, documentos, dados bancários e status da conta Stripe Connect antes de liberar operação financeira.
                    </small>
                @endif
            </div>
        </div>
