...
class ApiClient extends GetxService {
  ...
  void updateHeader(String? token, String? languageCode, {String? zoneId}) {
    _mainHeaders = {
      'Content-Type': 'application/json; charset=UTF-8',
      AppConstants.localizationKey: languageCode ?? AppConstants.languages[0].languageCode!,
      'zoneId': zoneId??'${sharedPreferences.getString(AppConstants.cacheZoneId)}',
  ...
  Response handleResponse(http.Response response, String uri, bool handleError) {
    ...
    if(response0.statusCode != 200 && response0.body != null && response0.body is! String) {
      if(response0.body is Map && response0.body.toString().startsWith('{errors: [{code:')) {
        ErrorResponse errorResponse = ErrorResponse.fromJson(response0.body as Map<String, dynamic>);
        response0 = Response(statusCode: response0.statusCode, body: response0.body, statusText: errorResponse.errors![0].message);
      } else if (response0.body is Map && (response0.body as Map).containsKey('message')) {
        response0 = Response(statusCode: response0.statusCode, body: response0.body, statusText: (response0.body as Map)['message']?.toString());
      }
