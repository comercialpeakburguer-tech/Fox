...
class RegularOrderBottomView extends StatelessWidget {
  ...
  Widget _buildOrderWaitingStatus() {
    return Container(
      ...
      child: Text('A caminho da loja', style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeLarge), textAlign: TextAlign.center),
    );
  }
  ...
  void _handleOrderConfirmation(OrderController orderController, OrderModel order) {
    if (!_foxGoCanActWithinRadius(order, targetCustomer: false, actionLabel: 'chegar na loja')) {
      return;
    }
  ...
  void _handleOrderPickup(OrderController orderController, OrderModel order) {
    if (!_foxGoCanActWithinRadius(order, targetCustomer: false, actionLabel: 'concluir a coleta')) {
      return;
    }
  ...
  void _handleOrderDelivery(OrderController orderController, OrderModel order) {
    if (!_foxGoCanActWithinRadius(order, targetCustomer: true, actionLabel: 'chegar na entrega')) {
      return;
    }
  ...
  void _handleCompleteDelivery(OrderController orderController, OrderModel order) {
    if (!_foxGoCanActWithinRadius(order, targetCustomer: true, actionLabel: 'finalizar a entrega')) {
      return;
    }
  ...
  static const double _foxGoActionRadiusKm = 0.2;

  bool _foxGoCanActWithinRadius(OrderModel order, {required bool targetCustomer, required String actionLabel}) {
    final LatLng? target = _foxGoTargetLatLng(order, targetCustomer: targetCustomer);
    final currentLocation = Get.find<ProfileController>().recordLocationBody;

    if (target == null) {
      showCustomSnackBar('Não foi possível confirmar o endereço para $actionLabel. Atualize o pedido e tente novamente.');
      return false;
    }

    if (currentLocation?.latitude == null || currentLocation?.longitude == null) {
      showCustomSnackBar('Não foi possível confirmar sua localização atual. Verifique o GPS e tente novamente.');
      return false;
    }

    final double distanceKm = Get.find<AddressController>().getRestaurantDistance(target);
    if (distanceKm > _foxGoActionRadiusKm) {
      final int distanceMeters = (distanceKm * 1000).round();
      showCustomSnackBar('Você precisa estar a até 200 metros para $actionLabel. Distância atual: ${distanceMeters}m.');
      return false;
    }
  ...
  LatLng? _foxGoTargetLatLng(OrderModel order, {required bool targetCustomer}) {
    final double? latitude = _foxGoParseCoordinate(targetCustomer ? order.deliveryAddress?.latitude : order.storeLat);
    final double? longitude = _foxGoParseCoordinate(targetCustomer ? order.deliveryAddress?.longitude : order.storeLng);
  ...
  double? _foxGoParseCoordinate(String? value) {
