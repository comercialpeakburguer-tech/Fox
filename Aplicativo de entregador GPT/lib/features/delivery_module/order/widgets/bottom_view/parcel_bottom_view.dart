...
class ParcelBottomView extends StatelessWidget {
  ...
  Widget _buildParcelWaitingStatus() {
    return Container(
      ...
      child: Text('A caminho da coleta', style: robotoBold.copyWith(fontSize: Dimensions.fontSizeLarge), textAlign: TextAlign.center),
    );
  }
...
// Parcel action handlers
  ...
  void _handleParcelConfirmation(OrderController orderController, OrderModel order) {
    if (!_foxGoCanActWithinRadius(order, targetReceiver: false, actionLabel: 'chegar na coleta')) {
      return;
    }
  ...
  void _handleParcelPickup(OrderController orderController, OrderModel order) {
    if (!_foxGoCanActWithinRadius(order, targetReceiver: false, actionLabel: 'concluir a coleta')) {
      return;
    }
  ...
  void _handleParcelDelivery(OrderController orderController, OrderModel order) {
    if (!_foxGoCanActWithinRadius(order, targetReceiver: true, actionLabel: 'chegar na entrega')) {
      return;
    }
  ...
  bool _foxGoCanActWithinRadius(OrderModel order, {required bool targetReceiver, required String actionLabel}) {
    final LatLng? target = _foxGoTargetLatLng(order, targetReceiver: targetReceiver);
  ...
  LatLng? _foxGoTargetLatLng(OrderModel order, {required bool targetReceiver}) {
    final double? latitude = _foxGoParseCoordinate(targetReceiver ? order.receiverDetails?.latitude : order.deliveryAddress?.latitude);
    final double? longitude = _foxGoParseCoordinate(targetReceiver ? order.receiverDetails?.longitude : order.deliveryAddress?.longitude);
